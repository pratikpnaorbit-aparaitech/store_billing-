...
3. Enter every `sync: false` value when prompted: billing `MONGODB_URI`, shared-user `AUTH_MONGODB_URI`, `CORS_ORIGINS`, Cloudinary credentials, and either `BREVO_API_KEY` or SMTP credentials. Render generates `JWT_SECRET`.
