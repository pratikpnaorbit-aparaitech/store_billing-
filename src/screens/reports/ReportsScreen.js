...
export default function ReportsScreen({ navigation }) {
  ...
  const { stats, weekly, top, payments } = useMemo(() => {
    ...
    const weeklyData = days.map((date) => ({ day: date.locale(language).format('ddd'), amount: orders.filter((order) => dayjs(order.createdAt).isSame(date, 'day')).reduce((sum, order) => sum + Number(order.total || 0), 0) }));
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.content}><View style={styles.header}><TouchableOpacity onPress={() => navigation.goBack()} style={styles.back}><Ionicons name="arrow-back" size={22} color="#0F172A" /></TouchableOpacity><View><Text style={styles.title}>{t("Reports")}</Text><Text style={styles.subtitle}>{t("Live data from completed sales")}</Text></View></View><View style={styles.grid}><Metric label={t("Total sales")} value={formatCurrency(stats.totalSales)} /><Metric label={t("Average bill")} value={formatCurrency(stats.averageBill)} /><Metric label={t("Orders")} value={stats.totalOrders} /><Metric label={t("Items Sold")} value={stats.productsSold} /></View><SalesChart data={weekly} /><Text style={styles.section}>{t("Payment mix")}</Text><View style={styles.paymentCard}>{Object.entries(payments).map(([name,count])=><View key={name} style={styles.paymentRow}><Text style={styles.paymentName}>{t(name)}</Text><Text style={styles.paymentValue}>{count} {t("Orders")}</Text></View>)}</View><Text style={styles.section}>{t("Top products")}</Text>{top.length ? top.map((item)=><TopProductCard key={item.id} product={item}/>) : <Text style={styles.empty}>{t("Complete a sale to see product rankings.")}</Text>}</ScrollView>;
}
