...
export default function CustomerScreen() {
  ...
  const save = async () => {
    if (!name.trim()) return Alert.alert(t("Name required"), t("Enter the customer name."));
    if (phone && !/^\d{10}$/.test(phone)) return Alert.alert(t("Invalid mobile"), t("Enter a 10 digit mobile number or leave it blank."));
    try { await addCustomer({ name, phone }); setName(""); setPhone(""); setShowForm(false); }
    catch (error) { Alert.alert(t("Could not add customer"), error.message); }
  };

  const remove = (customer) => Alert.alert(t("Delete customer?"), customer.name, [
